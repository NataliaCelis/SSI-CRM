...
export default function StaffDirectory({ staff, onClose, onSave }) {
  const { isFullManager, staff: currentStaff } = useAuth();
  ...
  const inpCls = 'w-full bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-500 disabled:opacity-60';
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            ...
            <div key={s.id || i} className="bg-gray-50 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700 rounded-xl p-4 space-y-3">
              <div className="flex gap-3 items-start">
                <div className="flex-1 grid grid-cols-2 gap-3">
                  <input value={s.name} onChange={e => update(i, 'name', e.target.value)} disabled={!isFullManager} className={inpCls} placeholder="Name" />
                  <input value={s.email} onChange={e => update(i, 'email', e.target.value)} disabled={!isFullManager} className={inpCls} placeholder="email@company.com" />
                </div>
                {isFullManager && (
                  <button onClick={() => removeMember(i)} className="text-red-400 hover:text-red-300 text-lg leading-none mt-1 flex-shrink-0 transition-colors">×</button>
                )}
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {ALL_ROLES.map(r => (
                    <button key={r} type="button" onClick={() => isFullManager && toggleRole(i, r)}
                      className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${s.roles.includes(r) ? 'bg-orange-500 border-orange-400 text-white' : 'bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400'} ${!isFullManager ? 'cursor-default' : 'hover:border-orange-400'}`}>
                      {r}
                    </button>
                  ))}
                </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between gap-3">
          {isFullManager && (
            <button onClick={addMember} className="text-sm text-orange-500 hover:text-orange-400 transition-colors">+ Add Staff Member</button>
          )}
          <div className="flex gap-2 ml-auto">
            <button onClick={onClose} className="px-4 py-2 text-sm bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors text-gray-700 dark:text-gray-300">Cancel</button>
